...
public abstract class DictionaryDrawer<TK, TV> : PropertyDrawer
{
    ...
    private void AddNewItem()
    {
        ...
        if (typeof(TK) == typeof(string))
            key = (TK)(object)"";
        else if (typeof(TK).IsSubclassOf(typeof(UnityEngine.ScriptableObject)))
            key = (TK)Convert.ChangeType(ScriptableObject.CreateInstance(typeof(TK)), typeof(TK));
        else if (typeof(TK).IsSubclassOf(typeof(UnityEngine.Object)))
            key = (TK)Activator.CreateInstance(typeof(TK));
        else key = default(TK);
